...
1.  Is this an observational study or an experiment? The original research
    ...
    Ans: This is an obervational study.
...
The distribution is left skewed. This states that the students are giving high scores.
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    ```{R gitter}
    score <- jitter(evals$score, factor = 1, amount = NULL)
    plot(score ~ evals$age)
    ```
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    ```{R m_bty}
    m_bty <- lm(evals$score ~ evals$age)
    plot(score ~ evals$age)
    abline(m_bty)
    ```
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    ```{R risudual}
    #plot_ss(x = evals$age, y = score, showSquares = TRUE)
    ```
...
7.  P-values and parameter estimates should only be trusted if the
    ...
    Ans: Normal Residuals: The resdiuals are not normal
...
Ans: low p-value of 0.117. Indication of weak relationship.
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    summary(m_full)
    ```
    Removing number of professors changed the values.
...
Variables linearly related to the outcome:
```{R linear}
...
17. The original paper describes how these data were gathered by taking a
    ...
    Ans:No. the class courses are independent, therefore the scores from the courses should be independent.


18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.
    Ans: Non minority male would be associated with a high score.



19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    Ans: No becasue this is an observational study. The results will certainly vary from university to university.

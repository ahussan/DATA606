...
Subject: Inference for categorical data
---
...
Ans: These percentages appear to be sample statistics.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
Ans: We assume that the smaples were randomly taken from difference sources around the world. This seems like a reasonaable assumption
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Ans: Each row in table 6 corresponds to a country and corresponding percentage for each answer choice.
...
atheistInUs <- subset(atheism, atheism$response=="atheist" & atheism$nationality=="United States" & atheism$year=="2012")
nrow(atheistInUs)/nrow(us12)
```
Ans: It agrees with the percentage in Table 6.
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    Ans: The smaple size is large and the observations are independent. Hence all conditions are met.
...
```
```{R France}
france <- subset(atheism, nationality == "France" & year == "2012")
inference(france$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

```{R ME France}
MEFrance <- (0.3089-0.2657)/2
MEFrance
...
Ans: When the population proportion moves closer to .5 from both side the margin of error increases. ME is hightest when Population Proportion is .5
...
```{R sum}
 summary(p_hats)
```
The distribution is normal with a center around .1
...
10. Repeat the above simulation three more times but with modified sample
    ...
    The three distributions are normal. P moves the center of the distribution and n changes the spread of the distribution.
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    Ans: Since the distributions are normal and samples are large, it is sensible to proceed with inference and report margin of errors.
...
an overlap in the intervals for Spain in 2005 and 2012 is present, hence we can say that there is not enough evidence that establish that there is a difference in the athesim index for those years.
...
```
Intervals are not overlapping for the year 2005 and 2012, hence there is enough evidence to say there is a difference in the athesim index for those years.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    Ans: Type 1 error is rejecting H0 when H0 is actually true. We would expect 5% of the countries to have a change.

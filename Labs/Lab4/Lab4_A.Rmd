...
```{r histarea, eval=TRUE}
hist(area)
```

###Exercise: 1
####Describe this population distribution.
This distribution is unimodal with a maximum in the 1000-1500 sq.ft area and right skewed, Max area is 5642
...
```{r sum-samp1, eval=TRUE}
summary(samp1)
```

```{r histsam1, eval=TRUE}
hist(samp1)
```

These data are also unimodel and right skewed a highest value is 3222.
...
```{r samp3 hist, eval=TRUE}
hist(samp3)
```
```{r samp4, eval=TRUE}
samp4 <- sample(area, 100)
mean(samp4)
summary(samp4)
```

```{r samp4 hist, eval=TRUE}
hist(samp4)
```
The larger samples of N = 100 and 1000 have means of 1481.96 and 1452.27 sq ft, these are much closer to the population mean of 1500 sq ft than the N = 50 which have means of 1460.76 and 1412.66 sq ft. max observation of these sample is 3222 wherease population max is 5642. That deviated mean significantly. 
...
Ans:
There are 5000 elements in sample_mean50. The sample distribution fits in the idea of a Normal Distribution with a mean of 1500 sq. ft., which is the population mean.

If we took 50,000 samples the Normal Distribtion would have a smaller standard deviation and still have a maximum at 1,500 sq. ft.
...
There are 100 elements in sample_means_small, each represents the mean of a sample of 50 houses from the areas list.
...
Ans: Center is unaffected, but spread decreases as sample size increases.
* * *
## On your own
...
```{r sample_means50 mean, eval=TRUE}
mean(sample_means50)
mean(price)
sd(sample_means50)

```
####Sample Mean: [1] 180618.6

####Population Mean: [1] 180796.1

Mean appears to be 180618.6, shape is unimodel based on this sampling distribution. Population mean is 180796.1
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    ```{r samprice simulate 150, eval=TRUE}
...
```
```{r sample_means150 mean, eval=TRUE}
...
The larger the sample size, the smaller the spread. We want to make our sample size as large as resources allow to decrease the spread. That will increase our confidence that our sampling has close to the population mean.

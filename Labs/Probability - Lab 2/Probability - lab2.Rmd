...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    Ans: A streak of 1 is one hit and one miss. A streak of 0 is a single miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    Ans: The distribution is a mode where zero and one is right skewed.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    ```
```{R barplot of sim_unfair}
barplot(table(sim_unfair_coin))
```

In my simulation of flipping the unfair coin 100 times, heads came up 23 times.
...
```{r sim-basket size one, eval=TRUE}
outcomes <- c("H", "M")
...
```{R calc streak of sim_baskert}
streak_sim_basket <- calc_streak(sim_basket)
barplot(table(streak_sim_basket))
```
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    Ans: Somewhat similar. With 133 shoots the chances of it being totally different are very     small, but non-zero.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Ans: Kobe did not have “hot hands”.



http://rpubs.com/ahussan/309070
